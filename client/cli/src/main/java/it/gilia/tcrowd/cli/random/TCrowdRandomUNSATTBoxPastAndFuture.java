...
@Command(name = "RandomUNSATTBoxPastAndFuture",
description = " TBox -> QTL1 -> QTLN -> LTL "
				+ "\n"
				+ "\t \t \t \t UNSAT TBox is randomly generated with past and future operators and the required parameters. Output is a Pure Future LTL"
				+ "\n"
				+ "\t \t  \t \t \t * If ABox is empty, only TBox is checked for SAT"
				+ "\n"
        		+ "\t \t \t \t \t * option -s requires entering a solver name (NuSMV|Aalta|pltl|TRP++UC|all)")

public class TCrowdRandomUNSATTBoxPastAndFuture extends TCrowdRandomTDLRelatedCommand {
    ...
    public void run() {
        ...
        try {
    		...
    		TBox tbox = new TBox();
    		tbox = exTDLITE.getUnsatTBox(ltbox, lc, n, qm, pt, pr);
