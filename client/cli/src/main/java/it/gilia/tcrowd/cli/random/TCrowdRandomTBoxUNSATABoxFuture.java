...
@Command(name = "RandomTBoxUNSATABoxFuture",
description = " TBox| UNSAT ABox on N -> QTL1 -> LTL. No past to future translation is required"
			  + "\n"
			  + "\t \t \t \t Both TBox and UNSAT ABox with only future operators are randomly generated given the required parameters."
			  + "\n")

public class TCrowdRandomTBoxUNSATABoxFuture extends TCrowdRandomTDLRelatedCommand {
    ...
    public void run() {
        ...
        try {
    		...
    		TD_LITE_N_ABox exTDLITE_N = new TD_LITE_N_ABox();
    		TBox tbox = new TBox();
    		tbox = exTDLITE_N.getFTBox(ltbox, lc, n, qm, pt, pr);
                   	         	    
            TDLLiteFPXReasoner.buildFOCheckSatisfiabilityOnlyFuture(
                    		tbox,
                    		true, 
                    		"random",
                    		exTDLITE_N.getUnsatABox(aboxS, aboxM, aboxQ),
                    		true);
